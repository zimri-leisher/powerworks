...
fun Level.getBlockCollisions(x: Int, y: Int, width: Int, height: Int): Sequence<Block> {
    ...
    return getIntersectingBlocksFromRectangle(x, y, width, height).filter {
        Geometry.intersects(it.x + it.hitbox.xStart, it.y + it.hitbox.yStart, it.hitbox.width, it.hitbox.height,
                x, y, width, height)
    }
}
...
fun Level.getBlockCollisionsInSquareCenteredOn(x: Int, y: Int, radius: Int): Sequence<Block> {
    ...
    return getChunksFromRectangle(x - radius, y - radius, radius * 2, radius * 2).flatMap {
        ...
        Geometry.intersects(x - radius, y - radius, radius * 2, radius * 2,
...
fun <L : LevelObject> Iterable<L>.getCollisionsWith(x: Int, y: Int, width: Int, height: Int): Sequence<L> {
    ...
    return asSequence().filter {
        it.hitbox != Hitbox.NONE && Geometry.intersects(x, y, width, height,
                it.x + it.hitbox.xStart, it.y + it.hitbox.yStart, it.hitbox.width, it.hitbox.height)
    }
}
 ...
 */
fun Level.doHitboxesCollide(hitbox1: Hitbox, x1: Int, y1: Int,
                            hitbox2: Hitbox, x2: Int, y2: Int): Boolean {
    return Geometry.intersects(x1 + hitbox1.xStart, y1 + hitbox1.yStart, hitbox1.width, hitbox1.height,
