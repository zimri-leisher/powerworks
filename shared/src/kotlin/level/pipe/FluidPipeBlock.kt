    ...
    override fun render() {
        ...
        if (closedEnds[1])
            Renderer.renderTexture(
                Image.Block.PIPE_RIGHT_CLOSE,
                x + 16,
                y + (18 - Image.Block.PIPE_RIGHT_CLOSE.height) / 2
            )
        if (closedEnds[2])
            Renderer.renderTexture(Image.Block.PIPE_DOWN_CLOSE, x + 4, y - 5)
        if (closedEnds[3])
            Renderer.renderTexture(
                Image.Block.PIPE_LEFT_CLOSE,
                x - Image.Block.PIPE_LEFT_CLOSE.width,
                y + (18 - Image.Block.PIPE_LEFT_CLOSE.height) / 2
            )
