...
object GameResourceManager {
    ...
    fun getIdentifier(textureRegion: TextureRegion) =
            textureRegions.filter { it.value == textureRegion }.keys.firstOrNull()

    /**
     * Gets a texture from one of the loaded [TextureAtlas]
     * @param index the index of the texture (the number after the last underscore if there is one). -1 if no index or the
     * index is already in the identifier
     */
    ...
    /**
     * @param path the path relative to the compiled jar
