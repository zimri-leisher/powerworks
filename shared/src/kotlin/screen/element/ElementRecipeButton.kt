...
class ElementRecipeButton(parent: GuiElement, recipe: Recipe? = null, var onRecipeChange: (Recipe?) -> Unit = {}, var recipePredicate: (Recipe) -> Boolean = { true }) : GuiElement(parent) {
    ...
    override fun onInteractOn(interaction: Interaction) {
        ...
        GuiRecipeSelector.show(Recipe.ALL.filter(recipePredicate), Placement.Exact(absoluteXPixel, absoluteYPixel - GuiRecipeSelector.parentElement.heightPixels / 2),
    ...
    override fun render(params: TextureRenderParams?) {
        ...
        Renderer.renderDefaultRectangle(absoluteXPixel, absoluteYPixel, widthPixels, heightPixels, actualParams.combine(TextureRenderParams(brightness = 1.2f + if (mouseOn) 0.1f else 0.0f, rotation = 180f)))
        recipe?.iconType?.icon?.render(absoluteXPixel, absoluteYPixel, widthPixels, heightPixels, true, actualParams)
        super.render(params)
    }
