...
class ElementProgressArrow(parent: GuiElement, var from: GuiElement, var to: GuiElement,
    ...
    private fun updateAlignments() {
        start.xPixel = from.absoluteXPixel + getX(from, fromDir)
        start.yPixel = from.absoluteYPixel + getY(from, fromDir)
        end.xPixel = to.absoluteXPixel + getX(to, toDir)
        end.yPixel = to.absoluteYPixel + getY(to, toDir)
    }
