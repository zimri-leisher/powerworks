...
class SimplexNoise_octave(seed: Int) { // Simplex noise in 2D, 3D and 4D
    ...
    fun noise(xin: Double, yin: Double): Double {
        ...
        // coords
        val y1 = y0 - j1 + G2
        val x2 = x0 - 1.0 + 2.0 * G2 // Offsets for last corner in (x,y)
        // unskewed coords
    ...
    fun noise(xin: Double, yin: Double, zin: Double): Double {
        ...
        val y1 = y0 - j1 + G3
        val z1 = z0 - k1 + G3
        val x2 = x0 - i2 + 2.0 * G3 // Offsets for third corner in (x,y,z)
        // coords
        val y2 = y0 - j2 + 2.0 * G3
        val z2 = z0 - k2 + 2.0 * G3
        val x3 = x0 - 1.0 + 3.0 * G3 // Offsets for last corner in (x,y,z)
        // coords
    ...
    fun noise(x: Double, y: Double, z: Double, w: Double): Double {
        ...
        val z1 = z0 - k1 + G4
        val w1 = w0 - l1 + G4
        val x2 = x0 - i2 + 2.0 * G4 // Offsets for third corner in (x,y,z,w)
        // coords
        val y2 = y0 - j2 + 2.0 * G4
        val z2 = z0 - k2 + 2.0 * G4
        val w2 = w0 - l2 + 2.0 * G4
        val x3 = x0 - i3 + 3.0 * G4 // Offsets for fourth corner in
        // (x,y,z,w) coords
        val y3 = y0 - j3 + 3.0 * G4
        val z3 = z0 - k3 + 3.0 * G4
        val w3 = w0 - l3 + 3.0 * G4
        val x4 = x0 - 1.0 + 4.0 * G4 // Offsets for last corner in (x,y,z,w)
        // coords
    ...
    companion object {
        ...
        private val p_supply = shortArrayOf(151, 160, 137, 91, 90, 15, // this
                // contains
                // all the
                // numbers
                // between 0
