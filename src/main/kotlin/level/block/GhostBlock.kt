...
class GhostBlock(type: BlockType<*>, xTile: Int, yTile: Int, rotation: Int) : Block(type, xTile, yTile, rotation) {
    ...
    override fun render() {
        ...
        Renderer.renderTexture(texture.texture, xPixel - texture.xPixelOffset, yPixel - texture.yPixelOffset, TextureRenderParams(color = Color(1f, 1f, 1f, 0.4f)))
        Renderer.renderEmptyRectangle(xPixel, yPixel, type.widthTiles shl 4, type.heightTiles shl 4, params = TextureRenderParams(color = toColor(if (placeable) 0x04C900 else 0xC90004, 0.4f)))
